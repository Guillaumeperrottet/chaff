...
const mockDailyData: Record<string, Record<string, number>> = {
  "jeu. 22.05.25": {
    "Camping Lac": 1684.54,
    "Camping Pont": 291.69,
            ...
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd className="text-lg font-medium text-gray-900">
                    {dailyTotals[dailyTotals.length - 1]?.total.toLocaleString(
                      "fr-CH",
                      {
                        style: "currency",
                        currency: "CHF",
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  <tr key={mandate.id} className="hover:bg-gray-50">
                    ...
                    {dates.map((date) => (
                      <td key={date} className="table-cell text-right">
                        {mockDailyData[date]?.[mandate.name]?.toLocaleString(
                          "fr-CH"
                        ) || "0.00"}
                      </td>
                    ))}
                ...
                {dates.map((date) => (
                  <td key={date} className="table-cell text-right">
                    {mockMandates
                      ...
                      .reduce(
                        (sum, m) => sum + (mockDailyData[date]?.[m.name] || 0),
                        0
                      )
              ...
              <tr className="bg-gray-100 font-bold">
                ...
                {dates.map((date) => (
                  <td key={date} className="table-cell text-right">
                    {calculateTotal(mockDailyData[date] || {}).toLocaleString(
                      "fr-CH"
                    )}
                  </td>
                ))}
